...
@section('content')  
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <h2>Edit or Update Income Class </h2>
      ...
      <div class="row g-2">
        ...
        <div class="col-md p-2 py-2">
          ...
          <div class="form-floating py-2">
            <input type="text" class="form-control" name="category" id="floatingInputGrid" placeholder=" transaction date" value="{{$incomeCategory->category}}">
            <label for="floatingInputGrid">Expense Catgory</label>
          </div>
        
          <div class="form-floating py-2">
            <input type="text" class="form-control" name="description" id="floatingInputGrid" placeholder=" transaction date" value="{{$incomeCategory->description}}">
            <label for="floatingInputGrid">Description</label>
          </div>
          
          <div class="py-4 w-100">
          <button type="submit" class="btn btn-danger w-100  px-6 py-4">Update Income Category</button>
        </div>
        </form>
        </div>
          ...
          <tbody>
            @foreach($incomeCategories as $expCat)
            <tr>
              <td>{{$expCat->id}}</td>
              ...
              <td class="items-center d-flex justify-content-between ml-1">
                  ...
                  <form action="{{ route('expense-categories.destroy', $expCat->id) }}" method="POST">
                    @csrf
                    @method('DELETE')
